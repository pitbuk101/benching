...
from src.utils.engine import Engine, remove_limit_statement
...
import base64
import time
from src.env import WREN_UI_ENDPOINT

logger = get_custom_logger(__name__)

logger.info(f"WREN_UI_ENDPOINT: {WREN_UI_ENDPOINT}")

class WrenUI(Engine):
    def __init__(
        self,
        endpoint: str = WREN_UI_ENDPOINT,
        **_,
    ):
        ...
        **kwargs,
        ...
        try:
            ...
            async with session.post(
                ...
                logger.critical(
                    f"WRENUI Engine Request took {time.time() - start} seconds, response: {res}"
                )
