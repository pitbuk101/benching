...
class PostgresDatabaseFactory:
    ...
    async def insert(self, table_name, columns, values, schema):
        """
        ...
        if not columns or not values:
            ...
            return None
        cols = ', '.join(columns)
        row_val = ', '.join([f"'{item}'" for item in values])

        query = f"""INSERT INTO "{schema}"."{table_name}" ({cols}) VALUES ({row_val})"""
        logger.info(f"Insert Query: {query}")
        try:
            await self.pg.connect()
            # Wrap the row if it's a single row
            # row_values = values if isinstance(values[0], (list, tuple)) else [values]
            async with self.pg.pool.acquire() as conn:
                # await conn.executemany(query, row_values)
                await conn.executemany(query, values)
            logger.info(f"Inserted {len(values)} rows into {schema}.{table_name}.")
    ...
    async def update_status(self, table_name, upload_id, status, schema):
        """
        Update multiple rows in a table. Generic for any table/schema.
        """
        if not upload_id or not status:
            logger.error("Upload ID and status must be provided for update.")
            return None
        query = f"""UPDATE "{schema}"."{table_name}" SET status = '{status}' WHERE upload_id = '{upload_id}'"""
        logger.info(f"Update Query: {query}")
        try:
            await self.pg.connect()
            async with self.pg.pool.acquire() as conn:
                await conn.execute(query)
            logger.info(f"Updated rows in {schema}.{table_name}.")
