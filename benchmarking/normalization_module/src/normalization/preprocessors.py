...
from omegaconf import OmegaConf # Added for type hinting if config is passed directly

from src.common.utils import expand_abbreviations, normalize_inch_quotes, clean_text_for_llm
...
def apply_operations(df: pd.DataFrame, operations: List[Dict[str, Any]], 
                     ref_df: pd.DataFrame = None, config: OmegaConf = None) -> pd.DataFrame:
    """
    ...
    for op_config_node in operations: # op_config_node is an OmegaConf node
        ...
        params_from_yaml = {k: v for k, v in OmegaConf.to_container(op_config_node, resolve=True).items() if k != "type"}
        ...
        try:
            ...
            if op_type == "merge_with_reference":
                ...
                if "abbr_map" not in call_args and call_args.get("abbreviations_map_key") and config and hasattr(config, "abbreviations_maps"):
                    call_args["config_maps"] = OmegaConf.to_container(config.abbreviations_maps, resolve=True)
