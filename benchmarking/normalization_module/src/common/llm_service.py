...
class LLMClient:
    ...
    def __init__(self, config, logger: logging.Logger):
        ...
        api_key = self.llm_config.api_key_env_var
        base_url = self.llm_config.base_url_env_var

        self.client = OpenAI(api_key=api_key, base_url=base_url)
        import src.prompts.normalization_prompts as normalization_prompts
    ...
    def generate_text_completion(self, prompt_key: str, prompt_args: Dict, 
        ...
        if model is None: model = self.llm_config.model
        if temperature is None: temperature = self.llm_config.temperature
        ...
        retries = self.llm_config.get("max_retries", 3)
        timeout = self.llm_config.get("timeout_seconds", 120)
        last_error = None
