...
while ! nc -z $QDRANT_HOST 6333; do
    ...
    if [ $current -eq $TIMEOUT ]; then
        echo "Timeout: qdrant did not start within $TIMEOUT seconds, QDRANT_HOST=$QDRANT_HOST"
        exit 1
    fi
done
...
if [[ -n "$SHOULD_FORCE_DEPLOY" ]]; then
    ...
    while ! nc -z localhost $WREN_AI_SERVICE_PORT; do
        ...
        current=$((current + INTERVAL))
        if [ $current -eq $TIMEOUT ]; then
            echo "Timeout: wren-ai-service did not start within $TIMEOUT seconds, WREN_AI_SERVICE_PORT=$WREN_AI_SERVICE_PORT"
            exit 1
        fi
    done
    ...
    while ! nc -z $WREN_UI_HOST $WREN_UI_PORT && ! nc -z host.docker.internal $WREN_UI_PORT; do
        sleep $INTERVAL
        current=$((current + INTERVAL))
        if [ $current -eq $TIMEOUT ]; then
            echo "Timeout: wren-ui did not start within $TIMEOUT seconds looking at $WREN_UI_HOST $WREN_UI_PORT"
            exit 1
        fi
    done
