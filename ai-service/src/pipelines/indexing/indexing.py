...
#         user_question, solution_example = row
#         data.append({"user_question": user_question, "solution_example": solution_example})
#     return data
...
@async_timer
@observe(capture_input=False, capture_output=False)
async def index_sql_examples(
    eg_file_path: str,
) -> Dict[str, Any]:
    # data = read_xlsx(eg_file_path)
    data = read_query_files(eg_file_path)
    documents = [
        Document(content=item["user_question"], meta={"solution_example": item["solution_example"], "question_type": item["question_type"], "tenant": item["tenant"]})
        for item in data
    ]
    # documents_written = await document_store.write_documents(documents=documents, policy=DuplicatePolicy.OVERWRITE)
    return {"documents": documents}

@async_timer
@observe(capture_input=False, capture_output=False)
async def embed_sql_descriptions(
    index_sql_examples: Dict[str, Any],
    document_embedder: Any,
) -> Dict[str, Any]:
    return await document_embedder.run(index_sql_examples["documents"])

@async_timer
@observe(capture_input=False)
async def write_sql_description(
    embed_sql_descriptions: Dict[str, Any], sql_description_writer: DocumentWriter
) -> None:
    return await sql_description_writer.run(
        documents=embed_sql_descriptions["documents"]
    )
## End of Pipeline
...
class Indexing(BasicPipeline):
    def __init__(
        ...
        **kwargs,
        ...
        sql_examples_store = document_store_provider.get_store(dataset_name="sql_examples")


        self._components = {
            "cleaner": DocumentCleaner(
                [dbschema_store, view_store, table_description_store, sql_examples_store]
            ),
            ...
            "sql_document_store": sql_examples_store,
            "sql_description_writer": AsyncDocumentWriter(document_store=sql_examples_store, policy=DuplicatePolicy.OVERWRITE),
            "eg_file_path": os.environ.get("EXAMPLES"),
        }
    ...
    def visualize(self, mdl_str: str, id: Optional[str] = None) -> None:
        ...
        self._pipe.visualize_execution(
            ["write_dbschema", "write_view", "write_table_description","write_sql_description" ],
