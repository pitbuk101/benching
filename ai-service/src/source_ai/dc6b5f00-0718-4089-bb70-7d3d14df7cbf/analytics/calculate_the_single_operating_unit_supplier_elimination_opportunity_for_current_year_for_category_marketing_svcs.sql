...
SELECT
    YEAR,
    CATEGORY,
    SUM(TOTAL_SPEND) AS SINGLE_OU_SUPPLIER_SPEND,
    SUM(SINGLE_OPERATING_UNITS_COUNT) AS COUNT_OF_SINGLE_OU_SUPPLIER,
    MAX(5/100) AS BENCHMARK_VALUE,
    SUM((TOTAL_SPEND * (5/100))) AS SINGLE_OU_SUPPLIER_ELIMINATION_OPPORTUNITY
FROM
    SINGLE_COUNT_DATA
GROUP BY 
    YEAR,
