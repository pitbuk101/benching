with CurrentQuarterData AS (
select
	min(CAST(DIM_DATE AS DATE)) AS min_date,
	max(CAST(DIM_DATE AS DATE)) AS max_date
from
	data.VT_DIM_PERIOD vdp
where
	YEAR(vdp.DIM_DATE) = YEAR(CURRENT_TIMESTAMP)
	and DATEPART(QUARTER, vdp.DIM_DATE) = QUARTER(CURRENT_TIMESTAMP)
),
PlantSpendSelectedYear AS (
SELECT
	plant.TXT_PLANT AS SelectedYearPlants,
	AVG(f.MES_SPEND_CURR_1) AS SelectedYearSpend,
...
WHERE
	TO_DATE(p.DIM_DATE, 'YYYYMMDD') >= (SELECT min_date from CurrentQuarterData)
	AND
	TO_DATE(p.DIM_DATE, 'YYYYMMDD') <= (SELECT max_date from CurrentQuarterData)
...
SELECT
	...
	AVG(f.MES_SPEND_CURR_1) AS PreviousYearSpend,
...
JOIN data.VT_C_DIM_SourcingTree_TECHCME ct ON
	ct.DIM_SOURCING_TREE = f.DIM_SOURCING_TREE
WHERE
	TO_DATE(p.DIM_DATE, 'YYYYMMDD') >= (SELECT DATEADD(MONTH,3, min_date) from CurrentQuarterData)
	AND
	TO_DATE(p.DIM_DATE, 'YYYYMMDD') <= (SELECT DATEADD(MONTH,3, max_date) from CurrentQuarterData)
	AND ss.DIM_SUPPLIER_STATUS = 'E'
	AND vt.DIM_VALUE_TYPE = 'I'
	AND ct.TXT_CATEGORY_LEVEL_2 = 'Bearings'
GROUP BY
