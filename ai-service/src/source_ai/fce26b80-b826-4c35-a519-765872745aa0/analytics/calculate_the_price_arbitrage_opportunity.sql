WITH BASE_TABLE AS (
    ...
    WHERE
        YEAR(TO_DATE(B.DIM_DATE, 'YYYYMMDD')) = YEAR(CURRENT_DATE)
...
SELECT
    YEAR,
    CATEGORY,
    ROUND(SUM(PRICE_ARBITRAGE), 2) AS PRICE_ARBITRAGE_OPPORTUNITY
FROM
    PRICE_ARB_TBL
GROUP BY
    YEAR,
    CATEGORY
ORDER BY
    PRICE_ARBITRAGE_OPPORTUNITY DESC;