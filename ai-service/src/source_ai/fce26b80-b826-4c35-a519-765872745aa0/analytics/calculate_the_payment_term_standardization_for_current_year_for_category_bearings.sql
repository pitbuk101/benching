WITH PAYMENTDETAILS AS (
    ...
    WHERE vdv.DIM_VALUE_TYPE = 'P'
        ...
        AND YEAR(TO_DATE(VDP.DIM_DATE, 'YYYYMMDD')) = YEAR(CURRENT_DATE) AND LOWER(SOUR.TXT_CATEGORY_LEVEL_2) = LOWER('Bearings')
    GROUP BY
...
SELECT
    ...
    PaymentTermGroup AS Payment_Term_Group,
    COALESCE(MAXPAYMENTDAYS, 90) AS Desired_Payment_Term_Days,
    0.1 AS WACC,
    PD.MAXPAYMENTDAYS,
    PD.SPEND,
    CASE
