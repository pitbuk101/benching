...
CONTRACT_SUP_DATA AS (
    SELECT
        YEAR,
        CATEGORY,
        SUPPLIER,
        COUNT(CONTRACT_END_DATE) AS CONTRACT_END_DATE_COUNT
    FROM
        CONTRACT_DATA
    WHERE
    ...
    GROUP BY
        ...
        CATEGORY,
        SUPPLIER
    HAVING COUNT(CONTRACT_END_DATE) > 1
)
SELECT
    YEAR,
    CATEGORY,
    COUNT(DISTINCT SUPPLIER) AS SUPPLIERS_WITH_MULTIPLE_CONTRACT_END_DATE,
    SUM(CONTRACT_END_DATE_COUNT) AS TOTAL_CONTRACT_END_DATE
FROM 
    CONTRACT_SUP_DATA
GROUP BY
    YEAR,
