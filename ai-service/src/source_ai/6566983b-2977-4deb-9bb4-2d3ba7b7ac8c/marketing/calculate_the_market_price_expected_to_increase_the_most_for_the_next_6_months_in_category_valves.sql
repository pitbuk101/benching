WITH
    ...
    NEXT_6_MONTHS_QUARTER_DATA AS (
        ...
        WHERE
            TO_DATE (DIM_DATE, 'YYYYMMDD') >= (
                SELECT
                    DATEADD (MONTH, 6, MIN_DATE)
                FROM
                    CURRENT_QUARTER_DATA
            )
            AND TO_DATE (DIM_DATE, 'YYYYMMDD') <= (
                SELECT
                    DATEADD (MONTH, 6, MAX_DATE)
                FROM
                    CURRENT_QUARTER_DATA
            )
            AND LOWER(TXT_CATEGORY) = LOWER('Valves')
	...
	FORECAST_6_MONTH_DATA AS (
		SELECT
			...
			ROUND(NEXT_PRICE, 2) AS NEXT_6_MONTH_QUARTER_PRICE,
			ROUND(
				(((NEXT_PRICE - PRICE) / PRICE) * COMPONENT_SHARE) * 100,
				2
			) AS NEXT_6_MONTH_COMPONENT_FORECAST,
			CONCAT (
				ROUND(
					(((NEXT_PRICE - PRICE) / PRICE) * COMPONENT_SHARE) * 100,
					2
				...
				'%'
			) AS NEXT_6_MONTH_COMPONENT_FORECAST_PERCENTAGE
		FROM
            SELECTED_QUARTER_DATA SQD
        JOIN NEXT_6_MONTHS_QUARTER_DATA NMD ON SQD.COMPONENT = NMD.COMPONENT
...
SELECT
	...
	SUM(NEXT_6_MONTH_QUARTER_PRICE) AS NEXT_6_MONTH_QUARTER_PRICE,
	SUM(NEXT_6_MONTH_COMPONENT_FORECAST) AS NEXT_6_MONTH_COMPONENT_FORECAST,
	ROUND(SUM(NEXT_6_MONTH_COMPONENT_FORECAST), 2) AS NEXT_6_MONTH_COMPONENT_FORECAST_PERCENTAGE
FROM
	FORECAST_6_MONTH_DATA