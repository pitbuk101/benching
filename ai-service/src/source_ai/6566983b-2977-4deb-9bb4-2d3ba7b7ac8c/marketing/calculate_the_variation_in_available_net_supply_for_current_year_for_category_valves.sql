WITH
    ...
    CURRENT_QUARTER_DATA AS (
        SELECT
            ...
            MAX(MES_VALUE) AS CURRENT_NET_SUPPLY
        FROM
    ...
    PREVIOUS_QUARTER_DATA AS (
        SELECT
            ...
            TXT_BEROE_SUBCATEGORY AS BEROE_CATEGORY,
            TXT_CRITERIA AS CRITERIA,
            MAX(MES_VALUE) AS PREVIOUS_NET_SUPPLY
        FROM
            DATA.T_C_MARKET_ANALYSIS
        WHERE
            LOWER(TXT_CRITERIA) LIKE LOWER('%Available Net Supply%')
...
SELECT
    ...
    ROUND(
        (
            CAST(CURRENT_NET_SUPPLY AS DECIMAL) - CAST(PREVIOUS_NET_SUPPLY AS DECIMAL)
        ) / CAST(PREVIOUS_NET_SUPPLY AS DECIMAL),
        2
