WITH
  ...
  (
    ...
    AND TO_DATE(P.DIM_DATE,'YYYYMMDD') >= DATEADD(MONTH, -3, CURRENT_DATE)
    GROUP BY
  ...
  LAST_3_MONTH_SPEND AS (
    ...
    AND TO_DATE (P.DIM_DATE, 'YYYYMMDD') <= DATEADD(MONTH, -3, CURRENT_DATE)
    AND TO_DATE (P.DIM_DATE, 'YYYYMMDD') >= DATEADD(MONTH, -6, CURRENT_DATE)
    GROUP BY
    YEAR(TO_DATE(P.DIM_DATE,'YYYYMMDD')),
      SOUR.TXT_CATEGORY_LEVEL_2,
      S.TXT_SUPPLIER
...
FROM THREE_MONTH_SPEND FMS
JOIN LAST_3_MONTH_SPEND LMS
