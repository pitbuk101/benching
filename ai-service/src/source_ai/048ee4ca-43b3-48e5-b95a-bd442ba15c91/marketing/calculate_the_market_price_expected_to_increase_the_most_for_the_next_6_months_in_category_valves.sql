...
NEXT_6_MONTHS_QUARTER_DATA AS (
    ...
    FROM
        DATA.T_C_MARKET_PRICES AS MP
    WHERE
        TO_DATE(DIM_DATE, 'YYYYMMDD') >= (SELECT DATEADD(MONTH,6, MIN_DATE) FROM CURRENT_QUARTER_DATA)
        AND TO_DATE(DIM_DATE, 'YYYYMMDD') <= (SELECT DATEADD(MONTH,6, MAX_DATE) FROM CURRENT_QUARTER_DATA)
        AND LOWER(TXT_CATEGORY) = LOWER('Valves')
    GROUP BY
        YEAR(TO_DATE(DIM_DATE, 'YYYYMMDD')),
        QUARTER(TO_DATE(DIM_DATE, 'YYYYMMDD')),
...
SELECT
	...
	ROUND(NEXT_PRICE,2) AS NEXT_6_MONTH_COMPONENT_PRICE,
	ROUND(((NEXT_PRICE-PRICE)/ PRICE)* 100, 2) AS NEXT_6_MONTH_COMPONENT_FORECAST,
	CONCAT(ROUND(((NEXT_PRICE-PRICE)/ PRICE)* 100, 2), '%') AS NEXT_6_MONTH_COMPONENT_FORECAST_PERCENTAGE
FROM
	SELECTED_QUARTER_DATA SQD
JOIN NEXT_6_MONTHS_QUARTER_DATA NMD
	ON
