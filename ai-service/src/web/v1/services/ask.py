...
class AskRequest(BaseModel):
    ...
    mdl_hash: Optional[str] = Field(default=None, validation_alias=AliasChoices("mdl_hash", "id"))
    ...
    async def ask(
        ...
        **kwargs,
        ...
        try:
            ...
            tenant_id  = ask_request.tenant_id
            if not self._is_stopped(query_id):
                self._ask_results[query_id] = AskResultResponse(
                    status="understanding",
                )
                logger.critical("Skipping Intent Pipeline")
                # TODO: Overriding Intent
                ...
                logger.critical("Running Query Stablization Pipeline")
                start = time.time()
                result = await self._pipelines["query_stablization"].run(
                    query=ask_request.query,
                    tenant_id=tenant_id,
                )
                logger.critical(f"Query Stablization time: {time.time() - start}")
                logger.debug(f"Stablized Query: {result}")
                stablized_query = result.get("post_processor").fixed_query.lower()
                # SAI-511 Introducing Redis Cache
                # redis_hostname = os.environ.get("REDIS_HOSTNAME", "localhost")
                # redis_port = os.environ.get("REDIS_PORT", 6379)
                # redis_password = os.environ.get("REDIS_PASSWORD")
                # r = redis.Redis(host=redis_hostname, port=redis_port, password=redis_password)
                # if r.exists(stablized_query):
                    # logger.debug(f"Cache hit for query: {stablized_query}")
                    # result = r.get(stablized_query)
                    # sql = result.decode("utf-8")
                    # api_results = 
            ...
            if not self._is_stopped(query_id):
                ...
                normalized_query = (
                                    ...
                                    .replace("batteries","[category]")
                                    .replace("'","")
                                )
                start = time.time() # NOSONAR
                retrieval_result = await self._pipelines["retrieval"].run(
                    query=stablized_query,
                    history=ask_request.history,
                    id=ask_request.project_id,
                    tenant_id=tenant_id
                )
                logger.critical(f"Retrieval time: {time.time() - start}")
                documents, samples = retrieval_result.get("construct_retrieval_results", [])
                sample_questions = list(samples.keys())
                logger.debug(f"Retrieval Results: {documents}")
                logger.debug(f"Samples: {samples}")
                if not documents:
                    logger.exception(
                        f"ask pipeline - NO_RELEVANT_DATA: {ask_request.query} || {stablized_query}"
                    )
                    if not self._is_stopped(query_id):
                        self._ask_results[query_id] = AskResultResponse(
                            status="failed",
                    ...
                    return results
                start = time.time() # NOSONAR
                reranker_result = await self._pipelines["query_reranker"].run(
                    query=normalized_query,
                    samples=sample_questions,
                    documents=documents,
                    tenant_id=tenant_id
                )
                logger.critical(f"Reranking time: {time.time() - start}")
                reranker_response = reranker_result.get("post_processor")
                logger.debug(f"Reranked Query: {reranker_response}")

                # temp = "## To Solve the Query, we will follow these steps:\n"
                # for ele in reranker_response.response:
                #     temp += (
                #         f"### STEP : {ele['step']}\n"
                #         f"    - Explaination : {ele['reason']}\n"
                #         f"    - QUERY : {ele['sql']}\n"
                #     )

                # sampling_document = (
                #     "### QUERY GENERATION STRATEGY SUMMARY ###",
                #     f"{reranker_response.combined_strategy}",
                ...
                samples_list = []
                counter = 0
                for question_dict in reranker_response.response:
                    if question_dict["confidence"] > 0.7:
                        counter+=1
                        samples_list.append("\n".join([
                            f"\t#### Sample: {counter}",
                            f"\t\t##### Question: {question_dict["question"]}",
                            f"\t\t###### SQL Query: {samples[question_dict["question"]]}"
                            ]))
            ...
            if not self._is_stopped(query_id):
                ...
                logger.critical(f"Historical Questions time: {time.time() - start}")
                ...
                if historical_question_result:
                    ...
                    if ask_request.history:
                        ...
                        logger.critical(f"followup_sql_generation time: {time.time() - start}")
                    else:
                        start = time.time()
                        text_to_sql_generation_results = await self._pipelines[
                            "sql_generation"
                            ...
                            # samples=sampling_document,
                            exclude=historical_question_result,
                            project_id=ask_request.project_id,
                            configuration=ask_request.configurations,
                            tenant_id=tenant_id
                        )
                        logger.critical(f"sql_generation time: {time.time() - start}")
                    ...
                    if sql_valid_results := text_to_sql_generation_results[
                        ...
                        logger.critical(f"sql_correction time: {time.time() - start}")
