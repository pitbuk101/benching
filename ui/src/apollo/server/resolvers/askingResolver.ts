...
export class AskingResolver {
  ...
  public async generateThreadRecommendationQuestions(
    ...
    const accessToken = ctx.headers['rawHeaders'].filter((header: string) =>
      header.includes('Bearer'),
    );
    // console.log('Looking for this console here', { accessToken });
    const { threadId } = args;
  ...
  public async createAskingTask(
    ...
    // console to debug token issue if needed
    // console.log('0 Looking for this console here', {
    //   header: ctx.headers,
    // });
    const accessToken = ctx.headers['rawHeaders'].filter((header: string) =>
      header.includes('Bearer'),
    ...
    // console to debug token issue if needed
    // console.log('1 Looking for this console here', { accessToken });

    const { question, threadId } = args.data;
    const project = await ctx.projectService.getCurrentProject();

    const askingService = ctx.askingService;
    const data = { question, accessToken };
  ...
  public async cancelAskingTask(
    ...
    const { taskId } = args;
    const askingService = ctx.askingService;
    await askingService.cancelAskingTask(taskId, accessToken);
    return true;
  }
  ...
  public async getAskingTask(
    ...
    const { taskId } = args;
    const askingService = ctx.askingService;
    const askResult = await askingService.getAskingTask(taskId, accessToken);
  ...
  public async createInstantRecommendedQuestions(
    ...
    const { data } = args;
    const askingService = ctx.askingService;
    return askingService.createInstantRecommendedQuestions(data, accessToken);
  }
