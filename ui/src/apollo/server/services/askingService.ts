...
// import { IContext } from '@/apollo/server/types'; // Ensure the correct import path
...
export interface IAskingService {
  ...
  cancelAskingTask(taskId: string, accessToken: string): Promise<void>;
  getAskingTask(taskId: string, accessToken: string): Promise<AskResult>;
...
export class AskingService implements IAskingService {
  ...
  public async cancelAskingTask(
    ...
    try {
      await this.wrenAIAdaptor.cancelAsk(taskId, accessToken);
  ...
  public async getAskingTask(
    ...
    return this.wrenAIAdaptor.getAskResult(taskId, accessToken);
  }
  ...
  public async getInstantRecommendedQuestions(
    ...
    const response = await this.wrenAIAdaptor.getRecommendationQuestionsResult(
      queryId,
      accessToken,
    );
    return response;
