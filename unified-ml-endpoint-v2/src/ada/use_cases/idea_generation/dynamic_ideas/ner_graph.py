...
def extract_entities(state: NERState) -> NERState:
    ...
    prompt = get_history_based_ner_prompt()
    analytics_list = get_analytics_values(state["pg_db_conn"], state["category"])
    ...
    ai_output = llm_response.content.replace("json", "").replace("`", "")
...
def create_ner_pipeline() -> CompiledStateGraph:
    ...
    workflow.add_node("get_corrected_entities", get_corrected_entities)

    workflow.set_entry_point("extract_entities")
    workflow.add_edge("extract_entities", "get_corrected_entities")
    workflow.add_edge("get_corrected_entities", END)
