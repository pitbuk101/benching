...
def extract_supplier_name_from_user_query(user_query: str) -> str:
    ...
    try:
        ...
        log.info("LLM generated output for NF PPT has mistake in json format %s", exc)
...
def create_supplier_profile(
    ...
    suggested_prompts = [
        ...
        for match_val in extracted_data.head(negotiation_conf["num_similar_suppliers"])[
            "supplier_name"
        ]
        .unique()
        .tolist()
    ...
    if extracted_data.head(1)["cosine_distance"].values[0] <= 0.05:
        extracted_data = extracted_data[extracted_data["cosine_distance"] <= 0.05]
        # extracted_data.sort_values(
        #     by=["spend_ytd"],
        #     ascending=False,
        #     inplace=True,
        # )
        # extracted_data = extracted_data.astype(
        #     {"number_of_supplier_in_category": float},
        # )
        # supplier_name = extracted_data.head(1)["supplier_name"].squeeze()
        # supplier_profile = enrich_supplier_profile(
        #     extracted_data.loc[extracted_data["supplier_name"] == supplier_name],
        # )
        # insights = get_modified_insights(supplier_profile.pop("insights"))
        # objectives = get_modified_insights(supplier_profile.pop("objectives"))
        # log.info("Extracted insights %s", insights)
        # return supplier_profile, insights, objectives

        ##Above code modified supplier profile if cosine distance is less and name matches but now i dont have it i need to create it

        #Accepts supplier name and returns profile, insights, objectives
        return create_supplier_profile_from_kf(pg_db_conn,
                                            sf_client,
                                            supplier_details,
                                            category,
                                            supplier_name=extracted_data.head(1)["supplier_name"].squeeze(),
    ...
    raise NegotiationFactoryUserException(
        f"We could not find the exact {supplier_name} in {category},  for current year."
...
def create_supplier_profile_from_kf(
    ...
    # for d in supplier_profile['objectives']:
    #     serialized = json.dumps(d, sort_keys=True)  # Convert dict to string for comparison
    #     if serialized not in seen:
    #         seen.add(serialized)
    #         unique_data.append(d)
    # supplier_profile['objectives'] = unique_data
...
def enrich_supplier_profile(supplier_profile: pd.DataFrame) -> pd.Series:
    ...
    if "insights" in supplier_profile.columns and "objectives" in supplier_profile.columns:
        agg_dict["insights"] = lambda x: list(chain.from_iterable(x))
        agg_dict["objectives"] = lambda x: list(chain.from_iterable(x))
    agg_dict["payment_term_avg"] = np.nanmax
    ...
    supplier_profile["spend_last_year"] = round(
        supplier_profile.get("spend_last_year", 0),
        2,
    )
    currency_str = supplier_profile.pop("currency_symbol")
    supplier_profile["currency_symbol"] = negotiation_conf["currency_map"].get(
        currency_str,
        currency_str,
    )
    ...
    supplier_profile["percentage_spend_across_category_ytd"] = round(
        supplier_profile.get("percentage_spend_across_category_ytd", 0),
        1,
    )
    supplier_profile["single_source_spend_ytd"] = round(
        supplier_profile.get("single_source_spend_ytd", 0),
        2,
    )
    supplier_profile["spend_no_po_ytd"] = round(
        supplier_profile.get("spend_no_po_ytd", 0),
        2,
    )
    ...
    supplier_profile["percentage_spend_which_is_single_sourced"] = round(
        supplier_profile.get("single_source_spend_ytd", 0) / spend_denom,
        1,
    )
    supplier_profile["percentage_spend_without_po"] = round(
        supplier_profile.get("spend_no_po_ytd", 0) / spend_denom,
        1,
    )
...
def extract_supplier_details(
    ...
    """
    # schema= '920a2f73-c7db-405f-98ea-f768c6da864f'
    ...
    extracted_data = pd.DataFrame(
        pg_db_conn.search_by_vector_similarity(
            table_name=table,
            query_emb=generate_embeddings_from_string(supplier_name.lower()),
    ...
    log.info("Extracted data columns %s", extracted_data.columns)
    log.info("Extracted data %s", extracted_data.head(1).squeeze().to_dict().get("cosine_distance", 0))
    if len(extracted_data) == 0 or (
        extracted_data.head(1).squeeze().to_dict().get("cosine_distance", 0)
        > negotiation_conf["supplier_similarity_threshold"]
    ):
        log.info("Extracted data in %s", extracted_data.head(1).squeeze().to_dict().get("cosine_distance", 0))
        raise NegotiationFactoryUserException(
...
def extract_supplier_details_from_kf(
    ...
    Raises:
        ...
        cosine distance is above the similarity threshold.
    """
    query = f"""select DISTINCT SUPPLIER_ID, SUPPLIER from {negotiation_tables["supplier_details"]} WHERE SUPPLIER = '{supplier_name}' AND CATEGORY = '{category}'  AND  YEAR = (SELECT MAX(YEAR) FROM {negotiation_tables["supplier_details"]})"""
...
def get_supplier_profile_insights_objectives(
    ...
    # supplier_profile, insights, objectives = create_supplier_profile_from_kf(
    #     pg_db_conn,
    #     sf_client,
    #     supplier_details,
    #     category,
...
def get_supplier_profile(
    ...
    try:
    ...
    except NegotiationFactoryException:
        try:
        ...
        except NegotiationFactoryUserException:
...
def extract_objective_description(reference_data: dict[str, Any], objective: str) -> str | None:
    ...
    """
    try:
        objective_df: pd.DataFrame = reference_data[
            negotiation_conf["reference_tables"]["common"]["negotiation_references"]
        ]
        ...
        return (
            filtered_objective_df[filtered_objective_df["l1_objective_description"].notna()].iloc[
                0
            ]["l1_objective_description"]
            if filtered_objective_df.shape[0] > 0
            else None
        )
    # pylint: disable=W0718
    except Exception as exc:
        log.error("Error while extracting objective description %s", exc)
        return None
    # pylint: enable=W0718
...
def get_samples(
    ...
    if len(filtered_objective_df) > 0:
        filtered_list = filtered_objective_df.to_dict(orient="records")[0]["samples"]
        examples = [
            {"example": item.get(generation_type[:-1])}
            for item in filtered_list
            if item.get(generation_type[:-1])
        ]
        examples = random.choices(examples, k=3) if len(examples) > 0 else [{"example": ""}] #NOSONAR
        return examples
    return [{"example": ""}]
...
def get_argument_conversation_history(
    ...
    """
    Extracts chat history in arguments, including new negotiation \
        arguments, counter arguments, or rebuttal arguments
    Args:
        chat_history_from_db (list): Extracted chat history from \
            database
    Returns:
        (list[dict]) : a list of messages in OpenAI message format
    """
    messages = []
    for data in chat_history_from_db:
        # use string match for argument related conversation
        if data.get("response_type", "") == generation_type:
            if data.get("request", ""):
                messages.append({"role": "user", "content": data["request"].get("user_query", "")})
            if data.get("response", ""):
                elements = data["response"].get(generation_type, [])
                if len(elements) > 0:
                    # this part should be similar / same to the
                    # defined llm output, so it can follow the format
                    assistant_message = (
                        "```json"
                        + "\n"
                        + json.dumps(
                            {
                                "message": data["response"].get("response", ""),
                                generation_type: elements,
                            },
                            indent=4,
                        )
                        + "\n"
                    ...
                    assistant_message = data["response"].get("response", "")
...
def update_negotiation_details(
    ...
    **kwargs: Any,
    ...
    if len(generated_details) > len(ids):
        ids.extend([f"{round(time.time())}{i}" for i in range(len(ids), len(generated_details), 1)])

    data_formatted = [
        {"id": ids[i], "raw": data, "details": data} for i, data in enumerate(generated_details)
    ]
...
def fetch_insights_for_supplier(
    ...
    sku_list = pinned_elements.get("skus", [])
    sku_ids = [sku.get("id") for sku in sku_list if sku.get("id")]
    for insight in pinned_elements.get("insights", []):
        arg_analytics_name = insight.get("analytics_name", "") or ""
        arg_insight_objective = insight.get("insight_objective", "") or ""
        if (
            arg_analytics_name.lower() == "general insights"
            or arg_analytics_name in all_analytic_names
            or arg_insight_objective.lower() in objective_types_lower
        ):
            insight_skus = [
    ...
    log.info("Filtered insights %s", filtered_insights)
...
def filter_insight_by_reinforcement(
    ...
    return [insight for insight in insights if reinforcement in insight["insight_reinforcements"]]
...
def ensure_key_exist(
    ...
    if required_key not in source_dict:
        ...
        error_msg = f"{required_key} is missing in the Payload"
        log.error(error_msg)
        raise NegotiationFactoryException(error_msg)
    if return_element:
        return source_dict[required_key]
    return None
...
def extract_model_context(
    ...
    # for objective_type in pinned_objective_types:
    #     result = extract_objective_description(
    #         reference_data=reference_data,
    #         objective=objective_type,
    #     )
    #     result = result[0] if isinstance(result, list) else result
    #     if result is not None:
    #         objective_descriptions.append(result)
...
def get_negotiation_model_context(
    ...
    # pylint: enable=R0801
    filtered_insights = fetch_insights_for_supplier(
        objectives_in_action=objectives_in_action,
...
def get_prompts_to_be_removed(
    ...
    if generation_type == "negotiation_approach_cp":
        ...
        remove_list.extend(
            [
                set_buy_pos,
                change_buy_pos if ("change " in user_query.lower()) else set_buy_pos,
...
def get_supplier_profiles( #NOSONAR
    ...
    if all_supplier_data.empty:
        raise NegotiationFactoryUserException(
        f"We could not find the exact supplier name: {supplier_names[0]} in {category}, for current year."
        "Is the supplier you are looking for one of these?",
        suggested_prompts,
    )
    ...
    # all_supplier_data['insights'] = all_supplier_data['insights'].apply(
    #     lambda x: json.loads(x) if isinstance(x, str) else x
    # )
    # all_supplier_data['objectives'] = all_supplier_data['objectives'].apply(
    #     lambda x: json.loads(x) if isinstance(x, str) else x
    # )
