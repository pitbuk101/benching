...
SELECT 
    ...
    ROUND((COUNT(CASE WHEN SUPPLIER_COUNT = 1 THEN MATERIAL_ID END)/COUNT(MATERIAL_ID))*100,2) AS SINGLE_SOURCE_MATERIAL_PERCENTAGE,
    ROUND(SUM(CASE WHEN SUPPLIER_COUNT = 1 THEN TOTAL_SPEND ELSE 0 END),2) AS SINGLE_SOURCE_SPEND,
    ROUND(SUM(TOTAL_SPEND),2) AS TOTAL_SPEND,
    ROUND((SUM(CASE WHEN SUPPLIER_COUNT = 1 THEN TOTAL_SPEND ELSE 0 END)/SUM(TOTAL_SPEND))*100,2) AS SINGLE_SOURCE_SPENDL_PERCENTAGE
FROM MATERIAL_DATA
GROUP BY YEAR,
    CATEGORY
