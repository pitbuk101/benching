WITH
    CONTRACT_DATA AS (
        SELECT
            ...
            COUNT(DISTINCT DIM_CONTRACT_MATERIAL) OVER (PARTITION BY YEAR(TO_DATE (B.DIM_DATE, 'YYYYMMDD')), SC.TXT_CATEGORY_LEVEL_2) AS TOTAL_CONTRACT_MATERIALS
        FROM
...
MATERIAL_YEAR_COUNTS AS (
    SELECT 
        CATEGORY,
        MATERIAL_ID,
        MATERIAL,
        COUNT(DISTINCT YEAR) as YEARS_WITH_SPEND
...
SELECT 
    CD.CATEGORY,
    COUNT(DISTINCT MYC.MATERIAL_ID) AS CONTRACT_NON_MOVING_MATERIALS,
    MAX(TOTAL_CONTRACT_MATERIALS) AS TOTAL_CONTRACT_MATERIALS,
    ROUND(CASE WHEN MAX(TOTAL_CONTRACT_MATERIALS)>0 THEN (COUNT(DISTINCT MYC.MATERIAL_ID)/MAX(TOTAL_CONTRACT_MATERIALS))*100 ELSE 0 END, 2) AS CONTRACT_NON_MOVING_MATERIALS_PERCENTAGE
FROM CONTRACT_DATA CD
JOIN MATERIAL_YEAR_COUNTS MYC ON CD.MATERIAL_ID = MYC.MATERIAL_ID AND CD.MATERIAL = MYC.MATERIAL AND CD.CATEGORY = MYC.CATEGORY
WHERE LOWER(CD.CATEGORY) = LOWER('Valves')
AND CD.YEAR = YEAR(CURRENT_DATE)
GROUP BY 
    CD.CATEGORY