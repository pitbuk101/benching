...
SELECT 
    CD.CATEGORY,
    COUNT(DISTINCT MYC.MATERIAL_ID) AS CONTRACT_MOVING_MATERIALS,
    MAX(TOTAL_CONTRACT_MATERIALS) AS TOTAL_CONTRACT_MATERIALS,
    ROUND(CASE WHEN MAX(TOTAL_CONTRACT_MATERIALS)>0 THEN (COUNT(DISTINCT MYC.MATERIAL_ID)/MAX(TOTAL_CONTRACT_MATERIALS))*100 ELSE 0 END, 2) AS CONTRACT_MOVING_MATERIALS_PERCENTAGE
FROM CONTRACT_DATA CD
