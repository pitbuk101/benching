...
SELECT Year,
    ...
    ROUND(AVG(CASE WHEN TOTAL_SPEND=0 then 0 else Mes_diff_invoice_payment_date_weighted/TOTAL_SPEND end),2) as PAYMENT_DAYS_WEIGHTED,
	SUM(MES_DIFF_EARLY_PAYMENT) AS Diff_Early_Payment,
    SUM( CASE 
        WHEN dp.MES_DIFF_EARLY_PAYMENT < -3 THEN ROUND(dp.Total_Spend,2)
        ELSE 0
        END )AS SPEND_PAID_EARLY,
...
ORDER BY PAYMENT_DAYS_WEIGHTED DESC