...
WHERE
	TO_DATE(p.DIM_DATE, 'YYYYMMDD') >= CAST('2023-01-01' AS DATE)
	AND TO_DATE(p.DIM_DATE, 'YYYYMMDD') <= CAST('2023-03-31' AS DATE)
...
JOIN data.VT_C_DIM_SourcingTree_TECHCME ct ON
	ct.DIM_SOURCING_TREE = f.DIM_SOURCING_TREE
WHERE
	TO_DATE(p.DIM_DATE, 'YYYYMMDD') >= DATEADD(MONTH, 3, CAST('2023-01-01' AS DATE))
	AND TO_DATE(p.DIM_DATE, 'YYYYMMDD') <= DATEADD(MONTH, 3, CAST('2023-03-31' AS DATE))
	AND ss.DIM_SUPPLIER_STATUS = 'E'
	AND vt.DIM_VALUE_TYPE = 'I'
	AND ct.TXT_CATEGORY_LEVEL_2 = 'Bearings'
GROUP BY
