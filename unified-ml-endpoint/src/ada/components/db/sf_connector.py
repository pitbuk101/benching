...
from ada.utils.config.config_params import tenant_ids_region, conn_params

negotiation_conf = read_config("use-cases.yml")["negotiation_factory"]
negotiation_tables = negotiation_conf["tables"]
class SnowflakeClient:

    def __init__(self, secret_name='Snowflake_sourceai_creds', tenant_id=None):
        """
        ...
        self.connection_params = conn_params.get(self.tenant_id)

        if not self.connection_params:
            raise KeyError(f"Connection parameters for tenant ID '{self.tenant_id}' not found")
    ...
    def create_connection(self):
        """
        Establish a connection to Snowflake, using tenant-specific connection parameters.
        """
        if hasattr(self, 'connection_params') and self.connection_params:
            # Use tenant-specific connection parameters
            conn = snowflake.connector.connect(
                ...
                ocsp_fail_open= True
                # fetch_size=self.connection_params["fetch_size"]
            )
        else:
            # Fallback to default environment variables if connection params are not set
            conn = snowflake.connector.connect(
    ...
    def execute_query(self, query):
        ...
        try:
            ...
            result = cursor.fetchall()  # Get the result of the query
            return result

        except Exception as e:
            log.error(f"Error executing query: {e}")
            # Attempt to re-execute the query after setting the database
            cursor.execute(f'USE DATABASE "{self.connection_params["database"]}"')
            cursor.execute(query)
    ...
    def fetch_dataframe(self, query):
        ...
        try:
            ...
            # all_data.append(pd.DataFrame.from_records(first_batch, columns=columns))

            # while True:
            #     rows = cursor.fetchmany(batch_size)
            #     if not rows:
            #         break
            #     all_data.append(pd.DataFrame.from_records(rows, columns=columns))
            ...
            try:
                ...
                log.error(f"Fallback fetch_pandas_all also failed: {inner_e}")
                return pd.DataFrame()
    ...
    def get_tail_spend_supplier_data(  # NOSONAR
        ...
        tail_spend_query = f"""WITH
            ...
            AVG(spend_ytd) OVER () AS total_spend
            ...
            from TotalSpends  
                ...
                ORDER BY  percentage_spend_contribution  limit {num_records};"""
