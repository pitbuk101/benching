...
# pylint: enable=W0611
from ada.use_cases.idea_generation.prompts import get_intent_prompt,generate_top_ideas_prompt,generate_rca_prompt,generate_open_world_response_prompt
...
idea_model_conf = read_config("use-cases.yml")["idea_generation_chat"]["model"]
    ...
    if any(pinned_elements[key] for key in pinned_elements):
        ...
        if not value_list and request_type != "linked_insights":
            ...
            if request_type == 'ideas':
                ...
                response = generate_chat_response_with_chain(prompt,temperature=idea_model_conf["temperature"],model=idea_model_conf["model_name"])
...
def open_world(query,chat_history):
...
def run_idea_generation_v2(
    ...
    try:
        ...
        open_world_response = open_world(json_data.get("user_input"), chat_history) if request_type == "user_input" else ""
            
        pg_db_conn = PGConnector(tenant_id=json_data["tenant_id"])

        sf_client = SnowflakeClient(tenant_id=json_data["tenant_id"])
        ...
        if json_data.get("pinned_elements").get("pinned_insights")!=[]:
            ...
            context = sf_client.execute_query(f"""
                    SELECT ANALYTICS_NAME, SEGMENT, CATEGORY, INSIGHT, OBJECTIVE ,LINKED_INSIGHTS, 
                    RELATED_INSIGHTS,RCA_HEADING, RCA_DESCRIPTION,TOP_IDEAS FROM "DATA"."INSIGHTS_MASTER"
                    WHERE ID = '{pinned_elements.get("insight_id")}'""")
        ...
        elif json_data.get("general_info",{}).get("id") != "":

            context_dict = {}

            general_info = json_data.get("general_info",{})
            id = general_info.get("id","") if general_info else ""
            idea_description = general_info.get("idea_description","") if general_info else ""

            # if idea_description:
            context = sf_client.execute_query(f"""
                    SELECT LINKED_INSIGHTS, 
                    RELATED_INSIGHTS,RCA_HEADING, RCA_DESCRIPTION,TOP_IDEAS FROM "DATA"."T_C_TOP_IDEAS"
                    WHERE ID = '{id}'""")
        ...
        user_input_response = generate_chat_model_response(
            ...
            open_world_response = open_world_response
