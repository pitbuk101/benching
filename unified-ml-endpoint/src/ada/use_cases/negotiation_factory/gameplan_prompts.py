...
def common_strategy_prompt(
    ...
    log.info("Generating common strategy prompt")
    log.debug("Category Positioning: %s", category_positioning)
    log.debug("Supplier Positioning: %s", supplier_positioning)
    log.debug("Market Approach: %s", market_approach)
    log.debug("Pricing Methodology: %s", pricing_methodology)
    log.debug("Contract Methodology: %s", contract_methodology)
    log.debug("Supplier Profile Keys: %s", list(supplier_profile.keys()))
    ...
    log.info("Supplier profile string created")

    supplier_name = supplier_profile.get("supplier_name", "this company")
    category_name = supplier_profile.get("category_name", "category")

    strategy_prompt = f"""
            You are a procurement expert for {category_name}.
            ...
            {category_name} spend. For e.g. 100% single source spend means there are no
            alternative suppliers for the SKUs which {supplier_name} supplies.
            It does NOT mean its 100% of {category_name}'s single source spend.
            '''
            NOTE:
        ...
        """
    log.info("Strategy prompt generation completed for supplier: %s", supplier_name)
...
def negotiation_strategy_prompt(
    ...
    """
    log.info("Generating negotiation strategy prompt")
    log.debug("Inputs — Category: %s | Supplier: %s | Market: %s | Pricing: %s | Contract: %s",
              category_positioning, supplier_positioning, market_approach, pricing_methodology, contract_methodology)
    ...
    log.info("Common prompt generated for negotiation strategy")
    ...
    log.info("Negotiation strategy prompt template constructed successfully")
...
def negotiation_set_positioning_prompt(
    ...
    contract_methodology: str,
    supplier_profile: dict[str, Any],
    percentage_spend_on_catgory: float,
) -> PromptTemplate:
    ...
    log.info("Generating negotiation set positioning prompt")
    ...
    log.info(f"Common prompt: {common_prompt}")
    ...
    if category_positioning == "":
        ...
        strategy_prompt = f"""
            ...
            percentage_spend_on_catgory: {percentage_spend_on_catgory}
            ...
            **IMPORTANT**: Please ensure that your explanation does not reference suppliers or supplier-specific details. Focus solely on the category and its characteristics.  
            ...
            - **Strategic**: Categories with high percentage of total company spending (≥ 80%) and high supply market complexity. These are critical to operations and require long-term, strategic management and active risk mitigation strategies.
            - **Leverage**: Categories with high percentage of total company spending (≥ 80%) but low supply market complexity. These offer strong negotiating power and should be optimized for cost, quality, and efficiency.
            - **Bottleneck**: Categories with low percentage of total company spending (< 80%) but high supply market complexity. These pose a risk due to supply constraints or dependency, requiring careful category management and risk mitigation.
            - **Shop (Non-Critical)**: Categories with low percentage of total company spending (< 80%) and low supply market complexity. These are low-impact, transactional purchases that should be streamlined through automation or aggregation."""
...
def negotiation_change_positioning_prompt(**kwargs) -> PromptTemplate:
    ...
    Returns:
        prompt template to give negotiation strategy to user and answer follow-up questions
    """
    log.info("Generating negotiation change positioning prompt")
    all_category_pos = kwargs.get("all_category_positioning", [])
    all_supplier_pos = kwargs.get("all_supplier_positioning", [])
    pinned_elements = kwargs.get("pinned_elements", {})

    log.debug("Available category positionings: %s", all_category_pos)
    log.debug("Available supplier positionings: %s", all_supplier_pos)

    all_category_str = ", ".join(all_category_pos)
    all_supplier_str = ", ".join(all_supplier_pos)

    supplier_positioning_value = pinned_elements.get("supplier_positioning", {}).get("value", "")
    category_positioning_value = pinned_elements.get("category_positioning", {}).get("value", "")

    supplier_cta = (
        "Set supplier positioning" if supplier_positioning_value == "" else "Change supplier positioning"
    )
    category_cta = (
        "Set category positioning" if category_positioning_value == "" else "Change category positioning"
    )

    if_change_category_request_type = (
        "negotiation_approach_sp" if supplier_positioning_value == "" else "negotiation_strategy_change"
    )
    if_change_supplier_request_type = (
        "negotiation_approach_cp" if category_positioning_value == "" else "negotiation_strategy_change"
    )

    log.debug("Computed CTA for supplier: %s", supplier_cta)
    log.debug("Computed CTA for category: %s", category_cta)
    log.debug("Request type for changing category: %s", if_change_category_request_type)
    log.debug("Request type for changing supplier: %s", if_change_supplier_request_type)
    ...
    log.info("Negotiation change positioning prompt template successfully created")
    return prompt_template
...
def change_prompt(
    ...
    Returns:
        prompt template to give negotiation strategy to user and answer follow-up questions
    """
    log.info("Generating negotiation strategy change prompt")
    ...
    log.debug("All Category Positioning: %s", all_category_str)
    log.debug("All Supplier Positioning: %s", all_supplier_str)
    log.debug("All Market Approach: %s", all_market_str)
    log.debug("All Pricing Methodology: %s", all_pricing_str)
    log.debug("All Contract Methodology: %s", all_contract_str)
    ...
    log.info("Negotiation strategy change prompt template created successfully")
    return prompt_template
