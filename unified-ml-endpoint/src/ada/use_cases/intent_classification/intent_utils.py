...
def save_conversation_history(
    ...
    log.debug("Entering save_conversation_history: chat_id=%s, request_id=%s", chat_id, request_id)
    log.debug("save_response content: %s", save_response)
    ...
    try:
        ...
        log.info("Successfully saved conversation history for chat_id=%s, request_id=%s", chat_id, request_id)

    except psycopg2.IntegrityError as unique_violation_exception:
        # log the error for debugging purposes
        log.error(
            "Unique constraint violation while saving conversation history: chat_id=%s, request_id=%s, error=%s",
            chat_id,
            request_id,
            unique_violation_exception,
        )
